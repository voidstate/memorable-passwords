...
class MemorablePassword
{
	...
	public function __construct( $num_digits = 3, $capitalise_mode = self::CAPITALISE_MODE_ONE, $word_list_mode = self::WORD_LIST_MODE_ALL )
	{
	...
	public function generate()
	{
		...
		$random_position = mt_rand( 1, count( $char_array ) - 1 );
	 ...
	 */
	public function setDigitCount( $num_digits )
	{
		if( !is_int( $num_digits ) )
		{
			throw new \InvalidArgumentException( 'Number of digits must be an integer.' );
		}
	...
	{
		if( !in_array( $capitalise_mode, [ self::CAPITALISE_MODE_NONE, self::CAPITALISE_MODE_ONE, self::CAPITALISE_MODE_SOME ] ) )
		{
			throw new \InvalidArgumentException( 'Capitalise mode invalid.' );
		}
	...
	{
		if( !in_array( $word_list_mode, [ self::WORD_LIST_MODE_ALL, self::WORD_LIST_MODE_ABSTRACT_WORDS_ONLY, self::WORD_LIST_MODE_NOT_ABSTRACT, self::WORD_LIST_MODE_ALL_LOCATIONS, self::WORD_LIST_MODE_ONLY_UK, self::WORD_LIST_MODE_ONLY_WORLDWIDE ] ) )
		{
			throw new \InvalidArgumentException( 'Word list mode invalid.' );
		}
